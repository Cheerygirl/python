...
SimTim = 10
RioTim = len(DlqRio)
ChainNum = len(chainname['buyer_name'])
...
m = 0
for i in range(0, RioTim):
    for j in range(0, SimTim):
        SimDis['SimDlq'].iloc[m] = DlqRio[i]
        SimDis['SimTim'].iloc[m] = j
        Sampleamount = 0
        Overdueamount = 0
        for k in range(0, ChainNum):
            sample_paydayid = np.random.choice(DataNum, chainname['Bill_number'], replace=False)
            sample_payday = cdata['pay_day'].iloc[sample_paydayid, :]
            sample = cdata[cdata['buyer_name']==chainname['buyer_name']]
            sample['pay_day'] = sample_payday
            Sampleamount = Sampleamount + sum(sample.bill_amount)
            SimCtDlqM = SimCtDlqF(sample, chainname['buyer_name'].iloc[j])
            TermDlq = TermDlq.append(DltTermF(SimCtDlqM, DlqRio[i], ChainDlqTerm.Credit_term[ChainDlqTerm['buyer_name']==chainname['buyer_name'].iloc[j] | ChainDlqTerm['SimDlq']==DlqRio[i]], chainname['buyer_name'].iloc[k]), ignore_index=True)
