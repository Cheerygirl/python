...
ChainIndex = pd.DataFrame(ChainIndex,index=['Chain_name','SimDlq','Bill_number']).T
...
for i in range(0,len(DlqRio)):
    for j in range(0,SimTim):
        ...
        for k in range(0, len(chainname['buyer_name'])):
            sampleid = np.random.choice(DataNum, ChainIndex.Bill_number[ChainIndex['Chain_name']==chainname['buyer_name'].iloc[k]], replace=False)
            sample = cdata.iloc[sampleid, :]
            Sampleamount = Sampleamount+sum(sample.Bill_amount)
            SimCtDlqM = SimCtDlqF(sample, chainname['buyer_name'].iloc[k])
            TermDlq = TermDlq.append(DltTermF(SimCtDlqM, DlqRio[i], ChainDlqTerm.Credit_term[ChainDlqTerm['Chain_name']==chainname['buyer_name'].iloc[k] | ChainDlqTerm['SimDlq']==DlqRio[i]], chainname['buyer_name'].iloc[k]), ignore_index=True)
